...
class SpikeAnalysis_MultiRecording:
    ...
    def __wilcox_baseline_v_event_stats__(
        self, recording_name, recording, event, equalize, baseline_window, offset, exclude_offset, save
    ):
        """
        ...
        unit_baseline_firing_rates = self.__get_unit_event_firing_rates__(
            recording, preevent_baselines, equalize=(baseline_window + offset), pre_window=0, post_window=0
        )
        if exclude_offset:
    ...
    def fishers_exact_wilcox(
        self, event1, event2, equalize, event3=None, baseline_window=None, offset=0, exclude_offset=False, save=False
    ):
        """
        ...
        if event3 is None:
            df1 = self.wilcox_baseline_v_event_collection(
                event1, equalize, baseline_window, offset, exclude_offset, plot=False, save=False
            )
            df2 = self.wilcox_baseline_v_event_collection(
                event2, equalize, baseline_window, offset, exclude_offset, plot=False, save=False
            )
        else:
            df1 = self.wilcox_event_v_event_collection(event1, event3, equalize, plot=False, save=False)
            df2 = self.wilcox_event_v_event_collection(event2, event3, equalize, plot=False, save=False)
        ...
        if save:
            if event3 is None:
                self.ephyscollection.fishers_exact[f"{event1} v {event2}: {equalize}s, {baseline_window}s baseline"] = [
                    odds_ratio,
                    p_value,
                    sig1,
                    not_sig1,
            ...
            if baseline_window is None:
                self.ephyscollection.fishers_exact[f"{event1} v {event2}: {equalize}s, {event3} baseline"] = [
                    odds_ratio,
                    p_value,
                    sig1,
                    not_sig1,
    ...
    def wilcox_baseline_v_event_unit(
        self, recording_name, unit_id, events, equalize, baseline_window, offset, exclude_offset=False
    ):
        """
        plots event triggered average firing rates for units with significant
        wilcoxon signed rank tests (p value <0.05) for event v baseline window.
        ...
        for event in events:
            temp_master_df = self.wilcox_baseline_v_event_collection(
                event, equalize, baseline_window, offset, exclude_offset, plot=False, save=False
            )
        ...
        if plot:
            ...
            if d == 3:
                self.__PCA_EDA_plot_3D__(
                    transformed_matrix, PCA_event_key, equalize, pre_window, post_window, azim, elev
                )
        return pc_dict
    ...
    def avg_geo_dist(self, equalize, pre_window, percent_var, post_window=0, events=None):
        temp_pairwise_distances = {}
        is_first = True
        for record_name, recording in self.ephyscollection.collection.items():
            pc_dict = self.PCA_matrix_generation(equalize, pre_window, post_window, events, [record_name])
    ...
    def __PCA_for_decoding__(self, equalize, pre_window, post_window, no_PCs, events):
        ...
        for i in range(len(recording_indices)):
            ...
            for event in events:
                ...
                event_firing_rates = self.__get_event_firing_rates__(
                    recording_instance, event, equalize, pre_window, post_window
                )
    ...
    def trial_decoder(self, equalize, pre_window, post_window, num_fold, num_shuffle, no_PCs, events, mode, plot=True):
        ...
        for event in events:
            ...
            if mode == "glm":
                auc[event] = {"glm": [], "glm_shuffle": []}
                prob[event] = {"glm": [], "glm_shuffle": []}
            if mode == "rf":
                auc[event] = {"rf": [], "rf_shuffle": []}
                prob[event] = {"rf": [], "rf_shuffle": []}
            if mode == "both":
                auc[event] = {"glm": [], "rf": [], "glm_shuffle": [], "rf_shuffle": []}
                prob[event] = {"glm": [], "rf": [], "glm_shuffle": [], "rf_shuffle": []}
            for fold in range(num_fold):
                ...
                prob_glm_shuffle = []
                prob_rf_shuffle = []
                data_test, label_test, data_train, label_train = self.__train_test_split__(
                    fold, num_fold, data_pos, data_neg, num_pos, num_neg
                )
                for timebin in range(T):
                    pred_glm, pred_rf = self.__run_model__(
                        mode, timebin, data_train, data_test, label_train, label_test
                    )
                    if (mode == "glm") | (mode == "both"):
                ...
                for shuffle in range(num_shuffle):
                    ...
                    for timebin in range(T):
                        ...
                        if (mode == "glm") | (mode == "both"):
                            temp_prob_glm_shuffle.append(pred_rf)
                            temp_glm_shuffle.append(roc_auc_score(label_test, pred_rf[:, 1]))
        ...
        if plot:
            self.__plot_auc__(auc, equalize, pre_window, mode)
        return [auc, prob]

    def __train_test_split__(self, fold, num_fold, data_pos, data_neg, num_pos, num_neg):
        pos_fold = num_pos // num_fold
        neg_fold = num_neg // num_fold
        data_test = np.concatenate(
            (
                data_pos[:, :, fold * pos_fold : (fold + 1) * pos_fold],
                data_neg[:, :, fold * neg_fold : (fold + 1) * neg_fold],
            ),
            axis=2,
        )
    ...
    def __run_model__(self, mode, timebin, data_train, data_test, label_train, label_test):
        if (mode == "glm") | (mode == "both"):
            ...
            if mode == "glm":
                pred_rf = None
        if (mode == "rf") | (mode == "both"):
            model_rf = BaggingClassifier(
                estimator=DecisionTreeClassifier(class_weight="balanced"), n_estimators=100, random_state=0
            )
            model_rf.fit(data_train[timebin, :, :].T, label_train)
            pred_rf = model_rf.predict_proba(data_test[timebin, :, :].T)
            if mode == "rf":
                pred_glm = None
        return (pred_glm, pred_rf)

    def __plot_auc__(self, auc_dict, equalize, pre_window, mode):
        ...
        for key in auc_dict.keys():
            ...
            if (mode == "rf") | (mode == "both"):
                ...
                x = np.linspace(-pre_window, equalize, len(rf_avg))
